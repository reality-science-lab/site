...
- HOME 先頭の **NEWS / LECTURE グリッド / NEXT LECTURE** に新記事が出ること、記事ページが開けることを見る。これら3領域はコレクション駆動で記事を1枚足すだけで自動更新される（並び順は `src/lib/posts.ts`、差し込みは `src/pages/index.astro`）。
- **HOME の3領域は「編集が即反映」の例外**。記事ファイルは `index.astro` の import 対象ではなくビルド時に collection から差し込まれるため、**dev サーバーを起動したまま記事を足すと HMR が部分的にしか効かず、グリッドだけ旧 Vol のまま（NEWS は更新）といったズレが出る**ことがある。HOME の3領域を確認するときは **dev サーバーを再起動**する（記事ページ単体のプレビューは再起動不要）。確実なのは `npm run build`（常に全領域を再計算する）。
